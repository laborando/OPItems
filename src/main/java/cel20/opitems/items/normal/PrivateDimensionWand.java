...
public class PrivateDimensionWand {
    ...
    public static void event(final PlayerInteractEvent e) {
        ...
        //Anti double click --> automatically executed when changing cel20.dimensions
        ...
        if (!Celutis.doesWorldExist(wn)) {
            ...
            worldCreator.generator(new PrivateDimensionChunkGenerator());
    ...
    public static void saveData(final String filePath) {
        try {
            ...
            out.writeObject((Object) PrivateDimensionWand.players_in_customw);
            out.close();
        } catch (IOException e) {
    ...
    public static void loadData(final String filePath) {
        try {
            ...
            PrivateDimensionWand.players_in_customw = (List<String>) in.readObject();
            in.close();
        } catch (ClassNotFoundException | IOException ex2) {
            System.out.println("Creating new file for dimensional player data");
            PrivateDimensionWand.players_in_customw = new ArrayList<>();
            File f = new File(filePath);
            try {
                if (!f.createNewFile()) {
                    ex2.printStackTrace();
